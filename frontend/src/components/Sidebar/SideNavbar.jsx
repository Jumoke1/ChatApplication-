...
const LeftSideBar = ({ 
    ...
    //  Build profile photo URL using environment variable
    const getProfilePhotoUrl = (dm) => {
        if (dm?.profilePhoto && dm.profilePhoto.trim() !== '') {
            ...
            return `${API_BASE}${dm.profilePhoto}`;
        }
        return null;
    };
    ...
    return (
        <>
            <aside className="w-80 h-screen bg-[#f8fafb] border-r border-gray-200 flex flex-col relative">
                {/* Mobile close button  */}
                <button 
                    onClick={onCloseMobile}
                    className="md:hidden absolute top-2 right-2 text-gray-500 hover:text-gray-700 p-1 rounded z-10"
                        ...
                        {channelsOpen && (
                            <div className="mt-1 space-y-0.5">
                                {channels.map((channel) => {
                                    ...
                                    const messageCount = Array.isArray(propMessages) 
                                        ? propMessages.filter((msg) => {
